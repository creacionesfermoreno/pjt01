...
namespace E_DataLayer.CentroEntrenamiento
{
    public class CentroEntrenamiento_Presencial_HorarioClasesAsistenciasData
    {
        ...
        public List<CentroEntrenamiento_Presencial_HorarioClasesAsistenciasDTO> CentroEntrenamiento_uspListarPresencial_HorarioClasesAsistenciasGestion_Cheking(CentroEntrenamiento_Presencial_HorarioClasesAsistenciasDTO request)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("CentroEntrenamiento_uspListarPresencial_HorarioClasesAsistenciasGestion_Cheking", conn))
                {
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.Parameters.Add(new SqlParameter("@CodigoUnidadNegocio", System.Data.SqlDbType.Int, 10)).Value = request.CodigoUnidadNegocio;
                    cmd.Parameters.Add(new SqlParameter("@CodigoSede", System.Data.SqlDbType.Int, 10)).Value = request.CodigoSede;
                    cmd.Parameters.Add(new SqlParameter("@CodigoHorarioClasesConfiguracion", System.Data.SqlDbType.VarChar, 100)).Value = request.CodigoHorarioClasesConfiguracion;
                    ...
                    using (SqlDataReader oIDataReader = cmd.ExecuteReader(System.Data.CommandBehavior.SingleResult))
                    {
                        if (oIDataReader.HasRows)
                        {
                            while (oIDataReader.Read())
                            {
                                ...
                                if (!oIDataReader.IsDBNull(oIDataReader.GetOrdinal("ObtenerTiempoVencimiento")))
                                {
                                    itemDTO.ObtenerTiempoVencimiento = oIDataReader[oIDataReader.GetOrdinal("ObtenerTiempoVencimiento")].ToString();
                                }
